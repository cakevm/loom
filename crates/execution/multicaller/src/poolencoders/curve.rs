    ...
    pub fn encode_swap_in_amount_provided<PoolEnum: PoolEnumTrait + Pool + Clone + Eq + Send + Sync + Display + Debug + 'static>(
            ...
            SwapAmountType::Stack0 => {
                if in_native {
                    ...
                    let mut approve_opcode = MulticallerCall::new_call(
                        token_from_address,
                        &EncoderHelper::encode_erc20_approve(cur_pool.get_address(), U256::ZERO),
                    );
        ...
        if out_native {
            let mut weth_deposit_opcode =
                MulticallerCall::new_call_with_value(token_to_address, &EncoderHelper::encode_weth_deposit(), U256::ZERO);
